...
import { ONDC_CONFIG } from '../src/config/ondc.js'; // Note config name
...
async function run() {
    ...
    try {
        console.log('Connected to DB. Starting Direct Axios POST...');
        ...
        const payload = {
            ...
            message: {
                order: {
                    ...
                    items: [{ id: item.id }],
                    fulfillments: [{ id: item.fulfillmentId || "F1" }]
                }
            }
        };
        ...
        console.log(`Sending to BPP: ${item.bppUri}/select`);
        const response = await axios.post(`${item.bppUri}/select`, payload, {
            headers: { 'Authorization': authHeader, 'Content-Type': 'application/json' }
        });
        console.log('✅ BPP ACCEPTED SELECT PAYLOAD:', response.data);
    } catch (error) {
        console.error('\n❌ BPP REJECTED SELECT PAYLOAD:');
        if (error.response) {
